...
// Resolve posts directory relative to this file (works in Deno Deploy)
...
export async function getPosts(): Promise<Post[]> {
  ...
  for await (const entry of Deno.readDir(POSTS_DIR)) {
    ...
    const raw = await Deno.readTextFile(`${POSTS_DIR}/${entry.name}`);
...
export async function getPost(slug: string): Promise<Post | null> {
  try {
    const raw = await Deno.readTextFile(`${POSTS_DIR}/${slug}.md`);
    const { meta, body } = parseFrontmatter(raw);
    return {
      slug,
      title: meta.title ?? slug,
